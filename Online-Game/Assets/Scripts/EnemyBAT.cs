...
public class EnemyBAT : NetworkBehaviour
{
    ...
    private void EnemyLogic()
    {
        ...
        if (state == EnemyStateBat.Idle || state == EnemyStateBat.Patrol)
        {
            FindTarget();
        }

        if (state == EnemyStateBat.Patrol || state == EnemyStateBat.Chase)
        {
            // Set the animator parameter based on whether the enemy is moving or not
            animator.SetBool(OnMove, true);
        }
        else if (state == EnemyStateBat.Idle)
        {
            // Set the animator parameter based on whether the enemy is moving or not
