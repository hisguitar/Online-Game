...
public class Enemy : NetworkBehaviour
{
	...
	private void ChasePlayer(GameObject player)
	{
		if (player != null)
		{
			...
			// Check for collisions with other enemies during chase
			CheckForNearbyEnemies();
	...
	{
		// Define a radius for checking nearby enemies
		Collider2D[] nearbyEnemies = Physics2D.OverlapCircleAll(transform.position, enemyRadius, LayerMask.GetMask("Enemy"));

		foreach (var enemy in nearbyEnemies)
		{
			if (enemy.gameObject != gameObject) // Ignore itself
			{
				// Move away from the detected enemy
